...
var settings = {
    ...
    client_id: 'Test.Code',
    redirect_uri: 'http://localhost:8621/oidc-client-sample.html',
    post_logout_redirect_uri: 'http://localhost:8621/oidc-client-sample.html',
    response_type: 'id_token token',
    scope: 'openid scope1 api1',
...
function signinDifferentCallback(){
    client.createSigninRequest({ state: { bar: 15 }, redirect_uri: 'http://localhost:8621/oidc-client-sample-callback.html' }).then(function(req) {
        log("signin request", req, "<a href='" + req.url + "'>go signin</a>");
        if (followLinks()) {
            window.location = req.url;
        }
